...
| Commit intelligence | `gitmind.advancedCommitActions`, `gitmind.openCommitWorkspace`, `gitmind.openHealthReport`, `gitmind.commitComposer`, `gitmind.reviewChanges`, `gitmind.draftChoices` |
...
Commit Intelligence is disabled by default. **Commit Health** has its own Pro Settings card and enabling it also enables the required workspace capability; **GitMind: Open Commit Health Report** is available while Health is enabled. These actions do not create stashes, pull requests, releases, or remote objects. Commit Composer applies a local series only after its separate final confirmation and never pushes.
